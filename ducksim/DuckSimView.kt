...
class DuckSimView(private val duckPond: DuckPond) : JPanel() {
    ...
    private fun paintDuck(g: Graphics, duck: Duck, rect: Rectangle) {
        ...
        if(duck.isOnDSWC && duck.state === State.WELCOMING && !(duck.isFree)) {
            drawCenteredTextInRectangle(g, color = Color.RED, text = "Beware!", rectangle = rect, yOffset = 35)
            duck.swim()
        }
        ...
        // If the duck is on DSWC, put a cyan W in the lower left corner of the square.
        if(duck.isOnDSWC) {
